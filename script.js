...
function populateExamDropdowns() {
  ...
  ['umExam','anExam','mlExam','saExam'].forEach(id => {
    const el = document.getElementById(id); if(!el) return;
    el.innerHTML = '<option value="">— Select Exam —</option>' + filteredExams.map(e=>`<option value="${e.id}">${e.name}</option>`).join('');
  });
}
  ...
  reader.onload=e=>{
    try {
      ...
      data.forEach(row=>{
        ...
        const str=streams.find(s=>s.name.toLowerCase()===strName.toLowerCase());
...
function getStudentReport(stuId, examId) {
  ...
  const total  = examMarks.reduce((a,m)=>a+m.score,0);
  const mean   = exam.subjectIds.length ? total/exam.subjectIds.length : 0;
  const mGrade = getMeanGrade(mean/(subjectRows.reduce((a,r)=>a+(r.max||100),0)/exam.subjectIds.length||100)*8);
  const totalPoints = subjectRows.reduce((a,r)=>a+(typeof r.points==='number'?r.points:0),0);
  ...
  return { stu, exam, cls, stream, subjectRows, total, mean, mGrade, totalPoints, overallRank, streamRank, history };
}
      ...
      <div class="rf-section-body" style="padding:0">
        <table class="rf-marks-table">
          ...
          <tbody>
            ${rows}
            <tr class="total-row">
              <td colspan="2" style="text-align:right">TOTALS / MEAN</td>
...
function generateReport() {
  ...
  area.innerHTML = stuList.map(stu => {
    ...
    // If user manually typed a value in rpFeeBalance, use it as global override
    if (feeBalance !== '') autoFeeBalance = feeBalance;
    return buildReportHTML(d, finalCT, finalPR, nextOpen, schoolClosed, autoFeeBalance, feeNextTerm, autoFeeStatus);
  }).join('');
  ...
  try {
    ...
    stuList.forEach((stu, idx) => {
      ...
      if (feeBalance !== '') {
        ...
        doc.setTextColor(0,0,0); doc.setFont(undefined,'normal'); doc.setFontSize(8.5);
        doc.text(`Fee Balance This Term: KES ${parseFloat(feeBalance||0).toLocaleString()}`, 14, afterPRSig+5);
        doc.text(`Fees for Next Term: KES ${parseFloat(feeNextTerm||0).toLocaleString()}`, PW/2, afterPRSig+5);
      }
